...
import { normalizeThresholds, processNotificationSnapshot } from "./logic.mjs";
...
async function markDelivery(delivery: any, values: Record<string, unknown>) {
  const { data, error } = await supabase.from("notification_deliveries")
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data) throw new Error(`Notification delivery ${delivery.delivery_id} no longer owns its claim.`);
}
...
async function sendDelivery(delivery: any): Promise<boolean> {
  ...
  try {
    ...
    }, JSON.stringify(delivery.payload), { TTL: 3600, urgency: "normal" });
  } catch (error) {
    pushError = error;
  }
